...
public class AddressBookManager {
	...
	public List<Contact> readAddressBookData(IOService dbIo) {
		try {
			if (dbIo.equals(IOService.DB_IO))
				AddressBookManager.addressList = new AddressBookDBService().readData();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return AddressBookManager.addressList;
	...
	public List<Contact> readAddressBookContactData(IOService dbIo) {
		try {
			if (dbIo.equals(IOService.DB_IO))
				AddressBookManager.addressBookList = new AddressBookDBService().readContactData();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return AddressBookManager.addressBookList;

	}

	public List<Contact> getPersonDetailsBasedOnName(IOService dbIo, String name) {
		if (dbIo.equals(IOService.DB_IO)) {
			AddressBookManager.addressList = addressDBService.getPersonDetailsBasedOnNameUsingStatement(name);
...
// TODO Auto-generated method stub
	...
	public Contact getContactData(String name) {
		return AddressBookManager.addressBookList.stream().filter(contacts -> contacts.getFirstName().equals(name))
	...
	public List<Contact> getContactsBasedOnStartDateUsingPreparedStatement(IOService ioService, String startDate,
		...
		if (ioService.equals(IOService.DB_IO))
			AddressBookManager.addressBookList = addressDBService
					.getContactsBasedOnStartDateUsingPreparedStatement(startDate, endDate);
		return AddressBookManager.addressBookList;
	}

	public List<Contact> readContactDataDB(IOService dbIo, String city, String state) {
		if (dbIo.equals(IOService.DB_IO)) {
			AddressBookManager.addressBookList = addressDBService.readContactAddressData(city, state);
		}
		return AddressBookManager.addressBookList;
	}
